...
void Port_Init(const Port_ConfigType* ConfigPtr)
{
	...
	*/
	...
	/*configure GPIOAFSEL register to program bit as GPIO or alternate
		* if chose alternate then:
				*PMCx in GPIOPCTL must be specified
		*also there are GPIOADCCTL and GPIODMACTL can be used to make the pin:
				*ADC	(GPIOADCCTL)
				*uDMA trigger (GPIODMACTL)
	*/
	
	/*****************PIN MODE******************/
	if(ConfigPtr->PortPinMode == PORT_PINMODE_DIO)
	{
		CLEAR_BIT(*((u32 *)(PortAdd+GPIOAFSEL_OFFSET)), ConfigPtr->ChannelId);
	}
	else if(ConfigPtr->PortPinMode == 0);
	else
	{
		SET_BIT(*((u32 *)(PortAdd+GPIOAFSEL_OFFSET)), ConfigPtr->ChannelId);
		
		(*((u32 *)(GPIOPCTL_OFFSET + PortAdd))) |=(u32)(((u8)(ConfigPtr->PortPinMode)) <<(4*(ConfigPtr->ChannelId)));
	}
